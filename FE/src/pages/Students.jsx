...
import { Plus, Search, Edit2, Trash2, Download, X, Check } from 'lucide-react';

function StudentModal({ student, onClose, onSave }) {
    const [form, setForm] = useState(student || {
        id: '', name: '', class: classes[0], gender: 'Nam', dob: '', phone: ''
    });
                ...
                <div className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input className="input" value={form.id} onChange={e => handle('id', e.target.value)} placeholder="HS001" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Lớp</label>
                            <select className="input" value={form.class} onChange={e => handle('class', e.target.value)}>
    ...
    return (
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                học sinh {filterClass !== 'Tất cả' ? `lớp ${filterClass}` : 'trong hệ thống'}
            </div>
                        ...
                        <tbody>
                            {filtered.map(s => (
                                <tr key={s.id} className="table-row">
                                    ...
                                    <td className="p-4">
                                        <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded text-xs font-medium">{s.class}</span>
                                    </td>
