...
public class FaceAttendanceController {
    ...
    public ResponseEntity<?> checkFace(
        ...
        try {
            // 0. Kiểm tra vị trí GPS trước tiên
            Map<String, Object> gpsResult = attendanceService.gpsCheckIn(studentId, classId, lat, lng);
            if (!(boolean) gpsResult.get("valid")) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of(
                        "success", false,
                        "message", gpsResult.get("message"),
                        "distance", gpsResult.get("distanceMeters")));
            }
            ...
            boolean isInClass = student.getClasses().stream()
                    .anyMatch(c -> c.getId().equalsIgnoreCase(classId));

            if (!isInClass) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "Học sinh không thuộc lớp " + classId));
            }

            String storedEmbedding = student.getFaceEmbedding();
            if (storedEmbedding == null || storedEmbedding.isEmpty()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "Học sinh chưa có dữ liệu khuôn mặt mẫu"));
            }
            ...
            try {
                ...
                return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                        .body(Map.of("message", "Dịch vụ nhận diện khuôn mặt đang bận hoặc gặp lỗi kết nối", "error",
                                e.getMessage()));
            }
            ...
            // Tìm tiết học đang diễn ra hoặc sắp bắt đầu (trong vòng 30p trước và trong lúc
            // học)
            ...
            if (existing.isPresent()) {
                record = existing.get();
                // Nếu đã điểm danh rồi thì chỉ cập nhật nếu trạng thái mới tốt hơn (ví dụ từ
                // vắng sang có mặt)
            ...
            if (status == AttendanceRecord.AttendanceStatus.late) {
                record.setNote("Đi muộn qua App lúc " + now + " tại " + gpsResult.get("locationName"));
            } else {
                record.setNote("Điểm danh tại " + gpsResult.get("locationName"));
            }
            ...
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message",
                    status == AttendanceRecord.AttendanceStatus.late ? "Điểm danh MUỘN thành công"
    ...
    public ResponseEntity<?> registerFace(
        ...
        try {
            Optional<Student> studentOpt = studentRepository.findById(studentId);
            if (studentOpt.isEmpty())
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Không tìm thấy học sinh");
